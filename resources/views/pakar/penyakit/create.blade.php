...
@section('title', 'Tambah Penyakit Baru - SiPakar Padi')
...
<div class="flex flex-col w-full text-neutral-800">
    ...
    <div class="bg-gradient-to-b from-[#0A3D2A] to-[#1C6646] px-6 pt-8 pb-10 text-white relative">
        <div class="flex items-center gap-2 mb-4">
            ...
            <h1 class="text-xl font-bold">Tambah Penyakit Baru</h1>
        </div>
        <p class="text-white/80 text-xs font-light">
            Masukkan kode, nama penyakit, dan deskripsi singkat penyakit baru.
        </p>
    </div>
    ...
    <div class="px-6 -mt-4 relative z-10 flex flex-col mb-8">
        ...
        <form action="{{ route('pakar.penyakit.store') }}" method="POST" class="bg-white p-6 rounded-2xl border border-neutral-100 shadow-md flex flex-col gap-4">
            @csrf

            <div>
                <label class="text-xs font-semibold text-neutral-600 mb-1.5 block">Kode Penyakit</label>
                <input type="text" name="kode_penyakit" value="{{ old('kode_penyakit', $suggestedCode) }}" required
            ...
            <div>
                ...
                <input type="text" name="nama_penyakit" value="{{ old('nama_penyakit') }}" required placeholder="Contoh: Hawar Daun Bakteri"
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="4" placeholder="Jelaskan penyakit ini secara singkat..."
