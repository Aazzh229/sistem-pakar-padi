...
@section('title', 'Ubah Pengguna - SiPakar Padi')
...
<div class="flex flex-col w-full text-neutral-800">
    ...
    <div class="bg-gradient-to-b from-[#0A3D2A] to-[#1C6646] px-6 pt-8 pb-10 text-white relative">
        <div class="flex items-center gap-2 mb-4">
            ...
            <h1 class="text-xl font-bold">Ubah Akun</h1>
        </div>
        <p class="text-white/80 text-xs font-light">
            Perbarui data akun pengguna, ganti password, ubah peran, atau nonaktifkan status keanggotaan.
        </p>
    </div>
    ...
    <div class="px-6 -mt-4 relative z-10 flex flex-col mb-8">
        <div class="bg-white p-5 rounded-2xl border border-neutral-100 shadow-lg">
            ...
            <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="flex flex-col gap-4">
                @csrf
                ...
                <!-- Password (optional) -->
                <div class="flex flex-col gap-1.5">
                    <label for="password" class="text-xs font-bold text-neutral-700">Password Baru (Opsional)</label>
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <select id="role" name="role" required 
                            class="w-full bg-neutral-50 border border-neutral-200 text-sm rounded-xl py-3 px-4 focus:outline-none focus:ring-2 focus:ring-[#0E4E37] focus:bg-white transition"
                    >
                        <option value="user" {{ $user->role === 'user' ? 'selected' : '' }}>User (Petani)</option>
                        <option value="pakar" {{ $user->role === 'pakar' ? 'selected' : '' }}>Pakar (Dr/Expert)</option>
